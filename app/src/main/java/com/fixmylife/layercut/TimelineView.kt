...
class TimelineView(ctx: Context) : View(ctx) {
    ...
    private fun contentHeight(): Float {
        ...
        return rulerH + gap + n * (rowH + gap) + mainH + gap
    }
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        if (p != null) {
            val bl = blocks()
            for (b in bl) drawBlock(canvas, b)
            if (p.main.isEmpty()) {
    ...
    private fun drawBlock(canvas: Canvas, b: Block) {
        ...
        canvas.drawRect(r, if (b.overlay) overlayPaint else mainPaint)
        val bmp = thumbs[b.clip.id]
    ...
    private enum class Mode { NONE, SCROLL, TRIM_L, TRIM_R, MOVE, SCALE }
    ...
    private val longPress = Runnable {
        if (mode == Mode.NONE) {
            ...
            if (hit != null) {
                ...
                if (hit.overlay) beginDrag(hit.clip, Mode.MOVE)
            }
        }
    }
    ...
    private val scaleDetector = ScaleGestureDetector(ctx, object : ScaleGestureDetector.SimpleOnScaleGestureListener() {
        override fun onScaleBegin(detector: ScaleGestureDetector): Boolean {
            ...
            if (mode == Mode.TRIM_L || mode == Mode.TRIM_R || mode == Mode.MOVE) return false
            mode = Mode.SCALE
            return true
        }
    ...
    override fun onTouchEvent(e: MotionEvent): Boolean {
        ...
        when (e.actionMasked) {
            MotionEvent.ACTION_DOWN -> {
                ...
                if (mode == Mode.NONE) postDelayed(longPress, 380)
                parent?.requestDisallowInterceptTouchEvent(true)
            }
            ...
            MotionEvent.ACTION_MOVE -> {
                ...
                when (mode) {
                    ...
                    Mode.MOVE -> applyMove(((e.x - downX) / pxPerMs).toLong())
                    Mode.SCALE -> {}
                }
            }
            MotionEvent.ACTION_UP -> {
                ...
                when (mode) {
                    ...
                    Mode.TRIM_L, Mode.TRIM_R, Mode.MOVE -> listener?.onEdited()
                    else -> {}
                }
                mode = Mode.NONE
                dragClip = null
            ...
            MotionEvent.ACTION_CANCEL -> {
                removeCallbacks(longPress)
                if (mode == Mode.TRIM_L || mode == Mode.TRIM_R || mode == Mode.MOVE) listener?.onEdited()
                mode = Mode.NONE
                dragClip = null
            }
        }
    ...
    private fun applyMove(dms: Long) {
        val c = dragClip ?: return
        c.startMs = (origStart + dms).coerceAtLeast(0L)
        invalidate()
    }
